...
@attributes {vector *labels; vector *used_labels; vector *new_vars; vector *params; char *funcname; bool has_call;} stats
@attributes {vector *used_labels; vector* vars; vector *new_vars; tree* ast; vector *params; char *funcname;bool has_call;} stat
@attributes {vector *vars; tree* ast; vector *params; bool has_call;} exprs expr lexpr prefix_expr plus_expr star_expr and_expr term 
...
def: IDENTIFIER LEFT_PAREN pars RIGHT_PAREN END
   ...
   @{
       ...
       @gen @revorder(1) write_function_header(@IDENTIFIER.name@, @stats.params@->len + @stats.new_vars@->len, @stats.has_call@);
   @}
   ...
   @{
       ...
       @gen @revorder(1) write_multistage_function_header(@IDENTIFIER.name@, @stats.params@->len + @stats.new_vars@->len, @stats.has_call@, @pars.0.new_params@, @pars.1.new_params@);
   @}
   ;
...
stats: stat SEMICOLON
     ...
     @{
        ...
        @i @stats.0.has_call@ = @stats.1.has_call@ || @stat.has_call@;
...
lexpr: IDENTIFIER
     ...
     @{
         ...
         @i @lexpr.has_call@ = @term.has_call@ || @expr.has_call@;
...
exprs: expr
     ...
     @{
         ...
         @i @exprs.0.has_call@ = @exprs.1.has_call@ || @expr.has_call@;
...
plus_expr: term PLUS term
         @{
             ...
             @i @plus_expr.has_call@ = @term.0.has_call@ || @term.1.has_call@;
         ...
         @{
             ...
             @i @plus_expr.has_call@ = @plus_expr.1.has_call@ || @term.has_call@;
...
star_expr: term STAR term
         @{
             ...
             @i @star_expr.has_call@ = @term.0.has_call@ || @term.1.has_call@;
         ...
         @{
             ...
             @i @star_expr.has_call@ = @star_expr.1.has_call@ || @term.has_call@;
...
and_expr: term AND term
        @{
            ...
            @i @and_expr.has_call@ = @term.0.has_call@ || @term.1.has_call@;
        ...
        @{
            ...
            @i @and_expr.0.has_call@ = @and_expr.1.has_call@ || @term.has_call@;
...
term: LEFT_PAREN expr RIGHT_PAREN
    ...
    @{
        ...
        @i @term.has_call@ = false;
    @}
    | term LEFT_BRACKET expr RIGHT_BRACKET
    @{
        @i @term.0.ast@ = new_tree(OP_INDEX, @term.1.ast@, @expr.ast@);
        @i @term.0.has_call@ = @term.1.has_call@ || @expr.has_call@;
    ...
    @{
        @i @term.ast@ = new_call_leaf(@IDENTIFIER.name@, @exprs.ast@, @term.vars@->len);
        @i @term.has_call@ = true;
    ...
    @{
        @i @term.0.ast@ = new_closure_call_leaf(@term.1.ast@, @exprs.ast@, @term.0.vars@->len);
        @i @term.0.has_call@ = true;

        @reg @term.1.ast@->reg = @term.0.ast@->reg; @exprs.ast@->reg = next_reg(@term.0.ast@->reg);
    @}
    ;
